...
\begin{itemize}
	...
	\item Scaled attention divides the pre-softmax scores $QK^⊤$ by its head size. If $Q$ and $K$ have unit variance, this makes the scores roughly unit variance, so the softmax stays diffuse and doesn't saturate.
\end{itemize}
...
The attention mechanism assigns \textit{attention scores} (\ie \textit{weights}) to different parts of the input sequence, indicating how much each part contributes to the current output. In essence, the model \textbf{pays attention} to certain parts of the input more than others while processing each token in the sequence.
...
\begin{figure}[t]
	...
	\caption{An illustration of the self-attention. The final output is the weighted sum of the value vectors.}
\end{figure}
...
You can also view the scaling factor as a \textbf{temperature}:
\begin{align*}
	\operatorname{softmax}\!\left(\frac{QK^\top}{\tau}\right).	
\end{align*}
\begin{itemize}
	\item Bigger $\tau$: softer (more uniform) distribution.
	\item Bigger $\tau$: sharper distribution.
\end{itemize}
...
In autoregressive tasks (\eg language modeling), it is essential that when predicting a token at position $i$, the model does not peek at any tokens at positions $j>i$. \textit{Masked attention} ensures that each token only attends to tokens at the same or earlier positions. The masked attention is often referred to \textit{cross-attention}. This is just a self-attention in decoder.
...
If we use a single value and a single key over all queries, we can significantly reduce the memory usage for \textit{KV caching}. This is the basic idea of the \textit{multi-query attention} (MQA) and the \textit{grouped query attention} (GQA). However, MQA tends to lower the output quality as the model size increases. GQA achieved a balance between them. 
